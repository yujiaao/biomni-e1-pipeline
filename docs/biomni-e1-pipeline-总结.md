...
- **抓取论文**：2539 篇（目标 2500，状态 `done`，结束原因：target reached (2539 >= 2500)）
- **LLM 提取**：2582 篇完成，累计抽取实体 —— 任务 **6764**、工具 **6828**、数据库 **811**、软件 **274**（失败/空标记 0 篇）
- **跨论文去重聚合**：工具 **4203**、数据库 **472**、任务 **5701**、软件 **193**（同名/近义合并后）
- **工具注册表**：`config/tool_registry.json` 共 **4203** 条；生成可调用封装 `tools/*.py` **4244** 个
- **语义检索索引**：`data/aggregated/retrieval_index.jsonl` **4675** 条（本地 bge-small-zh-v1.5 向量）
- **提取耗时**：约 **2142.7** 分钟（断点续传，崩溃自动重启）
- **完成时间**：2026-07-23 23:18:22
